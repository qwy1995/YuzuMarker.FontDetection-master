...
parser.add_argument(
    ...
    "--checkpoint",
    type=str,
    default="/data5/qwy/name=4x-epoch=84-step=1649340.ckpt",
...
def recognize_font(image):
    ...
    with torch.no_grad():
        ...
        print("length prob",len(prob))
        print("font_list",len(font_list))
        
        top_index = torch.argmax(prob).item()
        
        # 返回识别结果
        return {
            font_list[top_index].path: float(prob[top_index])
        }

        indicies = torch.topk(prob, 9)[1]
        print("length indicate",len(indicies))

        return [
            # {font_list[i].path: float(prob[i]) for i in range(config.FONT_COUNT)},
            {font_list[i].path: float(prob[i]) for i in range(len(font_list))},
            # *[gr.update(value=font_demo_images[indicies[i]]) for i in range(9)],
            *[
                gr.update(
...
with gr.Blocks() as demo:
    with gr.Column():
        ...
        with gr.Row():
            ...
            out = gr.Label(num_top_classes=1, label="Output Font")
            # inp = gr.Image(type="pil", label="Input Image")
...
demo.launch(share=args.share, server_port=7861,server_name="127.0.0.1")
